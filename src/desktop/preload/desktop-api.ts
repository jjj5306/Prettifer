...
export function createDesktopApi(invoke: Invoke): DesktopApi {
  return Object.freeze({
    ...
    listFileHistory: (request: FileHistoryRequest) => invoke(
      DESKTOP_CHANNELS.listFileHistory,
      request,
    ) as Promise<ApiResult<FileHistoryPageDto>>,
    readFileCommit: (request: FileCommitRequest) => invoke(
      DESKTOP_CHANNELS.readFileCommit,
      request,
    ) as Promise<ApiResult<FileCommitChangeDto>>,
    cancelFileHistory: (request: CancelFileHistoryRequest) => invoke(
      DESKTOP_CHANNELS.cancelFileHistory,
      request,
    ) as Promise<ApiResult<null>>,
