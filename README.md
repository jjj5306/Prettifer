...
`CHANGELOG.md`는 **쓰는 사람이 읽는 글**입니다. 사양서가 아니라 게임 패치노트에 가깝게 씁니다.

- 뭐가 달라졌고 그래서 뭐가 편해지는지를 씁니다. 읽는 사람이 이 프로그램을 만들었다고 가정하지
  않습니다.
- 내부 이슈 번호, 브랜치 이름, 소스 파일 경로는 쓰지 않습니다.
- 화면에 영어로 뜨는 이름은 괄호로 같이 적습니다.
- 왜 그렇게 만들었는지 설명이 필요하면 개발 노트로 덧붙입니다. 특히 "이렇게 하는 게 당연해
  보이는데 왜 안 했나" 싶은 결정은 적어 두는 편이 좋습니다.
...
- diff를 좌우 보기와 세로(인라인) 보기로 전환 (전환해도 보던 위치 유지)
