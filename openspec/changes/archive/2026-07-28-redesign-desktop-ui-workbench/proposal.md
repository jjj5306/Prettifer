...
- 결과 계산은 원본 저장소와 분리된 임시 작업 공간에서 수행하고 원본 Git 설정과
  다른 작업 공간 등록을 유지한다.
- 실제 파일 충돌과 그 밖의 Git 실행 실패를 구분해 원인에 맞는 진단을 제공한다.
...
- `desktop-repository-navigation`: Commit History를 가장 오래된 커밋부터 최신
  커밋 순서로 표시한다.
- `desktop-composite-diff-review`: 추가 파일을 기준 화면 없이 전체 추가 내용
  하나로 표시하고 diff 준비 실패 시 생성된 편집 자원을 정리한다.
- `composite-diff-generation`: 임시 작업 공간을 원본 저장소와 격리하고 파일
  내용 처리 설정, 저장소 메타데이터와 오류 분류 계약을 유지한다.
