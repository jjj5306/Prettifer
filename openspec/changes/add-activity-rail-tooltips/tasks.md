...
## 5. 독립 File History 탐색 흐름

- [x] 5.1 비교 범위만으로 File History를 열고 전체 파일을 선택하는 실패 renderer 및 controller 테스트를 작성한다
- [x] 5.2 전체 파일 → 커밋 이력 → 커밋 변경 내용의 실패 Electron 테스트를 작성한다
- [x] 5.3 비교 대상 브랜치의 전체 파일 목록을 조회하고 기준점을 구분해 캐시하는 상태 및 controller 동작을 구현한다
- [x] 5.4 File History를 전체 폭의 파일, 이력, 변경 내용 3단계 화면으로 구현한다
- [x] 5.5 짧은 파일 이력 항목을 상단 콘텐츠 높이로 배치하고 단계별 복귀 동작을 구현한다
- [x] 5.6 관련 테스트, 전체 검증과 이슈 #96 진행 상태를 새 범위에 맞게 갱신한다
